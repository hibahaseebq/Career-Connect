...
import { Container, Typography, Grid, Avatar, Paper, Box, Divider, LinearProgress, Chip, Link, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Button, TextField, Badge, Tooltip, FormControlLabel, Switch,
} from "@mui/material";
...
const ProfilePage = ({mode, profilePic, handleProfilePicChange }) => {
  const [token, setToken] = useState(sessionStorage.getItem('token')); 
  const [editOpen, setEditOpen] = useState(false);
  const [profileData, setProfileData] = useState({ id: null, userId: null, fullName: null, userType: null, description: null, address: null, profilePicture: null, bannerPicture: null, latitude: null, longitude: null, summary: null, skills: null, education: null, experience: null, certifications: null, languages: null, interests: null, recommendations: null, accomplishments: null, country: null, region: null, district: null, collegeOrUniversity: null, employmentType: null, companyName: null, isStudent: null, phoneNumber: null, headline: null, bio: null, avatarURL: null, socialLinks: null, contactInfo: null, websiteLink: null, connections: null, selfEmployed: null, endorsements: null, createdAt: null, updatedAt: null,
  });
...
const updateSessionStorageAndEmit = (avatarURL) => {
  ...
  const fetchProfileData = async () => {
    try {
      ...
      console.log("🚀 ~ fetchProfileData ~ role:", role)
...
const handleSelfEmployedChange = (event) => {
  ...
  const handleSaveProfile = async () => {
    try {
      await api.put(`/profile/update/${profileData.profileId}`, profileData, {headers: {
...
const skills = profileData.skills ? profileData.skills.split(',') : [];
  ...
  <>
    {!token ?
      (
        <Container style={{paddingTop:"95px"}} maxWidth="lg">
      <Paper  className={`${mode==="light"? "":"dark-m-bg-second-color"}`} elevation={3} sx={{ py: 4, px: 3, mb: 4 }}>
        <Box
          sx={{
      ...
      ):( <Container style={{paddingTop:"95px", paddingBottom:"50px"}} maxWidth="lg">
      <Paper id='profile-page-container' className={`${mode==="light"? "":"dark-m-bg-second-color"}`}  sx={{ py: 4, px: 3,   }}>
        {/* Profile Banner */}
        ...
        {profileData.userId === tokenUserId &&  <IconButton
            style={{
              position: "absolute",
              top: "8px",
              right: "8px",
            ...
            <div className="naming-section">
              <div style={{flex:"2"}}>
                ...
                >
                   ...
                   {profileData.websiteLink &&<Typography style={{textDecoration:"none"}} mt={1} variant="body2" fontWeight={"bold"} mb={2}>
                        <Link href={`${profileData.websiteLink}`} style={{display:"flex", alignItem:"center"}}>
                            visit my website 
                            <span >
                        <li-icon  aria-hidden="true" type="link-external" className="text-view-model__external-hyperlink-icon" size="small"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" data-supported-dps="16x16" fill="currentColor" className="mercado-match" width="16" height="16" focusable="false">
               ...
               {profileData.selfEmployed && <li>
                    ...
                    <WorkIcon  style={{fontSize:"18px",  marginRight:"10px", color:'#4CAF50' }}/>Self Employed
                    </Typography>
                </li>}
                ...
                {profileData.collegeOrUniversity?.trim() && (
                  <li>
                    ...
                    <SchoolIcon style={{fontSize:"18px",marginRight:"10px", color:'#4CAF50' }}/>{profileData.collegeOrUniversity}
                    </Typography>
                  </li>
                )}
            </ul>
