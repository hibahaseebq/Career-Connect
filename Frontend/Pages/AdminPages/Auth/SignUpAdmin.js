...
import { TextField, Select, MenuItem, Button, Typography, Grid, Container, Snackbar, Alert,  } from '@mui/material';
...
const SignUpAdmin = () => {
  ...
  const [navigationPath, setNavigationPath] = useState('/admina');
  console.log("🚀 ~ SignUpAdmin ~ navigationPath:", navigationPath)
  ...
  const handleSubmitFirstStep = async (e) => {
    e.preventDefault();
    setLoading(true);
    const { email, password, fullName, country, region, district, privateKey } = firstFormData;
    
    // Check if required fields are empty
    if (!email || !password || !fullName || !country || !region || !district || !privateKey) {
        setLoading(false);
    ...
    try {
      ...
      console.log("🚀 ~ handleSubmitFirstStep ~ response:", response)
      ...
      setNavigationPath(response.data.navigate);
    ...
    return (
      <>
        ...
        <Grid container spacing={2}>
            <Grid item xs={12}>
              ...
              required 
                ...
                label="Private Key"
                name="privateKey"
                value={firstFormData.privateKey}
                onChange={handleInputChange}
              />
            </Grid>
        ...
        <Button onClick={handleSubmitFirstStep} variant="contained" color="primary">
          ...
          <Button style={{minHeight:"36px"}} onClick={handleSubmitFirstStep} variant="contained" color="primary">
  ...
  const renderEmailVerificationStep = () => {
        ...
        <Grid container spacing={2}  >
              ...
              <Grid item xs={12}>
                ...
                Sign Out{/* {loading ? <Loader color="white" /> : 'Go back'} */}
                </Button>
  ...
  return (
    <Grid container justifyContent="center">
      {!token && !emailConfirmed ? (
        ...
        <>
        {!emailConfirmed && <form style={{ width: "80%" }} onSubmit={handleSubmitEmailVerification}>
